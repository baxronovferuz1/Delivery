...
async def create_product(product: ProductModel, Authorize: AuthJWT = Depends()):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Enter valid access token"
        )
    ...
    if current_user.is_staff:
        create_new_product = Product(name=product.name, price=product.price)
...
async def product_list(Authorize: AuthJWT = Depends()):
    ...
    if current_user.is_staff:
        ...
        return HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only admin can see products list",
        )
...
async def product_by_id(id: int, Authorize: AuthJWT = Depends()):
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail="The user is not authorized,enter valid access token",
        )
    ...
    if current_user.is_staff:
        ...
        if product:
            ...
            raise HTTPException(
                ...
                detail=f"No product found for this id {id}",
            )

        return jsonable_encoder(response)

    raise HTTPException(
        status_code=status.HTTP_403_FORBIDDEN, detail="Only superadmin can you see"
    )
...
async def delete_product(id: int, Authorize: AuthJWT = Depends()):
    ...
    if current_user.is_staff:
        ...
        if product:
            ...
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"ID {id} is not found"
            )
    else:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Only superadmin can delete"
        )


@product_router.put("/{id}/update", status_code=status.HTTP_200_OK)
async def update_product(
    id: int, updated_data: ProductModel, Authorize: AuthJWT = Depends()
):
    ...
    if current_user.is_staff:
        ...
        if product:
            for key, value in updated_data.dict(
                exclude_unset=True
            ).items():  # exlclude_unset=True>agar productni nomi o'zgarsa faqat o'sha fieldni update qiladi(partial)
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Only superadmin can update"
        )
