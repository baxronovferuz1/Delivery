...
async def make_order(order: OrderModel, Authorize: AuthJWT = Depends()):
    ...
    new_order = Order(quantity=order.quantity, product=order.product_id)
    new_order.user = user
...
async def order_list(Authorize: AuthJWT = Depends()):
    ...
    if user.is_staff():
        ...
        raise HTTPException(
            ...
            detail="Only superadmin can see all orders",
        )  # The user is authorized, but does not have permission
    ...
    if current_user.is_staff:
        ...
        if order:
            ...
            raise HTTPException(
                ...
                detail=f"No order found for this id {id}",
            )
...
async def update_order(id: int, order: OrderModel, Authorize: AuthJWT = Depends):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Enter access token!"
        )
    ...
    if order_to_update.user != user:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You can not change other's order",
        )
